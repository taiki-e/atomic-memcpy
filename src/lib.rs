...
pub unsafe fn atomic_load<T>(src: *const T, order: Ordering) -> core::mem::MaybeUninit<T> {
    ...
    let val = unsafe { atomic_load_relaxed(src) };
    match order {
...
use atomic::{AtomicU16, AtomicUsize, Ordering};
...
#[cfg_attr(feature = "inline-always", inline(always))]
#[cfg_attr(not(feature = "inline-always"), inline)]
unsafe fn atomic_load_relaxed<T>(src: *const T) -> MaybeUninit<T> {
...
mod store {
    ...
    use super::atomic::{AtomicU8, AtomicUsize, Ordering};
